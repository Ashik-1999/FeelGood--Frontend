  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (data.notFound) {
        ...
        toast.error("Sorry, You are restricted to login", {
          toastId:id,
          theme:"light"           
        });
      }else{   
   ...
   <section className="bg-gray-50 min-h-screen flex items-center justify-center">
        ...
        <div className="bg-gray-100 flex rounded-2xl shadow-lg max-w-3xl p-5 items-center">
          ...
          <div className="md:w-1/2 px-8 md:px-16">
            <h2 className="font-bold text-2xl text-center text-[#002D74]">User Login</h2>
          ...
          {boolean &&  <svg onClick={passwordView} xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="gray" class="bi bi-eye-slash absolute top-1/2 right-3 -translate-y-1/2" viewBox="0 0 16 16">
            ...
            <div className="mt-3 text-xs flex justify-between items-center text-[#002D74]">
              ...
              <button onClick={()=>navigate('/signup')} className="py-2 px-5 bg-white border rounded-xl hover:scale-110 duration-300">Register</button>
            </div>
          </div>
          {/* image */}
